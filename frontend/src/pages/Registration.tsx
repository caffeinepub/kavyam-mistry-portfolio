...
export default function Registration({ onNavigateToLogin }: RegistrationProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (msg.includes('already in use') || msg.includes('already taken') || msg.includes('already exists')) {
        setError('Username already taken. Please choose a different one.');
      } else if (msg.includes('required') || msg.includes('empty') || msg.includes('Empty')) {
      ...
      <div className="max-w-md w-full">
        ...
        <div className="minecraft-title-box p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <Label htmlFor="reg-username" className="text-gray-300 text-sm font-semibold minecraft-text">
                Choose a Username
              </Label>
              <Input
                id="reg-username"
            ...
            <div>
              <Label htmlFor="reg-password" className="text-gray-300 text-sm font-semibold minecraft-text">
                Choose a Password
              </Label>
              <Input
                id="reg-password"
