...
import { LogOut, RefreshCw, Loader2, Download, Upload, Save, AlertCircle } from 'lucide-react';
...
import { useGetMaintenanceMode, useToggleMaintenanceMode } from '@/hooks/useQueries';
import { createBackup, restoreBackup, getBackupInfo, downloadBackup } from '@/utils/backup';
...
export default function Owner({ onLogout }: OwnerProps) {
  const { data: maintenanceMode = false, isLoading: maintenanceLoading } = useGetMaintenanceMode();
    ...
    <div className="min-h-screen bg-black">
      ...
      <header className="border-b-4 border-cyan-700 bg-black/90 px-6 py-4 flex items-center justify-between sticky top-0 z-40">
        <div>
          ...
          <p className="text-gray-500 text-xs minecraft-text mt-0.5">AKGamer4354 — Admin Panel</p>
        </div>
        <Button onClick={onLogout} variant="outline" size="sm" className="backup-button">
        ...
        <section>
          <div className="minecraft-box-medium p-6">
            ...
            <div className="flex items-center gap-6">
              <div className={`px-4 py-2 border-2 font-bold minecraft-text text-lg ${
                maintenanceMode
        ...
        <section>
          <div className="minecraft-box-medium p-6">
            ...
            <div className="flex flex-wrap gap-3">
              <Button onClick={handleCreateBackup} variant="outline" size="sm" className="backup-button">
                <Download className="w-4 h-4 mr-2" />
                Create Backup
              </Button>

              {backupInfo && (
                <Button onClick={handleDownloadBackup} variant="outline" size="sm" className="backup-button">
              ...
              {backupInfo && (
                <AlertDialog>
                  ...
                  <AlertDialogContent className="backup-dialog">
                    ...
                    <AlertDialogFooter>
                      <AlertDialogCancel className="backup-dialog-button">Cancel</AlertDialogCancel>
                      <AlertDialogAction
            ...
            {backupInfo && (
              <p className="text-xs text-gray-500 mt-3">Last backup: {backupInfo.timestamp}</p>
            )}
          </div>
        </section>
